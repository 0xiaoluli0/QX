...
const MAX_VIDEO = getNum('wetalk_video_count', 5);
const VIDEO_DELAY = getNum('wetalk_video_delay', 8000);
const ACCOUNT_GAP = getNum('wetalk_account_gap', 3500);
...
if (typeof $request !== 'undefined' && $request) {
  ...
  if (!email) {
    ...
    const customAlias = readPref('wetalk_capture_alias');
    let alias;
    if (customAlias && customAlias.trim()) {
      alias = customAlias.trim();
      try { $prefs.setValueForKey('', 'wetalk_capture_alias'); } catch (e) {}
    } else {
      alias = existed ? (store.accounts[accId].alias || email) : email;
    }
