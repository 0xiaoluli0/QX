...
const MAX_VIDEO = getNum('pingme_video_count', 5);
const VIDEO_DELAY = getNum('pingme_video_delay', 8000);
const ACCOUNT_GAP = getNum('pingme_account_gap', 3500);
...
if (typeof $request !== 'undefined' && $request) {
  ...
  const customAlias = readPref('pingme_capture_alias');
  let alias;
  if (customAlias && customAlias.trim()) {
    alias = customAlias.trim();
    try { $prefs.setValueForKey('', 'pingme_capture_alias'); } catch (e) {}
  } else {
